...
## 3.9 Cache Optimization — Cache Blocking, Tiling, SIMD, Prefetch
...
- **L3 (SIMD tay) chỉ được thêm 1.8×** sau khi đã unpack-once — vì phần lớn việc
  "khó" (giải nén) đã bị loại ở L2. Đây là bài học lớn: **sửa đúng vấn đề (compute
  hay bandwidth) trước, viết SIMD sau** — viết SIMD trước khi biết mình
  compute-bound hay memory-bound là phí công.
